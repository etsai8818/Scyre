...
addListeners = currentUserUid => {
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('AI').on('child_added', snap => {
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Machine Learning').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
...
render() {
    ...
    return (
                ...
                <h1 style={{ fontFamily: 'proxima-nova, sans-serif' }}>
                    Artificial Intelligence and Machine Learning</h1>
...
export default connect(mapStateToProps, { setCurrentChannel, setPrivateChannel, setExplorePage })(Artificial);