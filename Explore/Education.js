...
addListeners = currentUserUid => {
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('EdTech').on('child_added', snap => {
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Education').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Language Learning').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('University').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
...
export default connect(mapStateToProps, { setCurrentChannel, setPrivateChannel, setExplorePage })(Education);