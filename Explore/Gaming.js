...
addListeners = currentUserUid => {
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Esports').on('child_added', snap => {
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Gaming').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Gambling').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
...
export default connect(mapStateToProps, { setCurrentChannel, setPrivateChannel, setExplorePage })(Gaming);