...
addListeners = currentUserUid => {
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Business Development').on('child_added', snap => {
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('CRM').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Employment').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Event Management').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Human Resources').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Innovation Management').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Knowledge Management').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Lead Generation').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Quality Assurance').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Property Management').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Training').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
...
export default connect(mapStateToProps, { setCurrentChannel, setPrivateChannel, setExplorePage })(Hr);