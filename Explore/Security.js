...
addListeners = currentUserUid => {
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('National Security').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Security').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Network Security').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
...
export default connect(mapStateToProps, { setCurrentChannel, setPrivateChannel, setExplorePage })(Security);