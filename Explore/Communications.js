...
addListeners = currentUserUid => {
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Communications').on('child_added', snap => {
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('VoIP').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Optical Communication').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Unified Communications').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
    ...
    this.state.companyRef.orderByChild('industryType').equalTo('Telecommunications').on('child_added', snap => {
        if (currentUserUid !== snap.key) {
            let company = snap.val();
            company['uid'] = snap.key;
            loadedUsers.push(company);
...
export default connect(mapStateToProps, { setCurrentChannel, setPrivateChannel, setExplorePage })(Communications);